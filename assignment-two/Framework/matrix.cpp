                       ...
                       Vector3 m_Up)
{
    ...
    Vector3 w = (m_Eye - m_At)*(1/(m_Eye - m_At).getLength());
    Vector3 u = Vector3::CrossProduct(m_Up, w)*(1/(Vector3::CrossProduct(m_Up, w)).getLength());
    Vector3 v = Vector3::CrossProduct(w,u);
    Vector3 a = m_Eye;
    
    /**
     *  @todo assignment two
     *  Implement a view matrix using eye position, at position and the up vector;
     */

    elements[0] = u.x;     elements[4] = u.y;     elements[8] = u.z;     elements[12] = -1*(u.x)*(a.x) - (u.y)*(a.y) - (u.z)*(a.z);
    elements[1] = v.x;     elements[5] = v.y;     elements[9] = v.z;     elements[13] = -1*(v.x)*(a.x) - (v.y)*(a.y) - (v.z)*(a.z);
    elements[2] = w.x;     elements[6] = w.y;     elements[10] = w.z;    elements[14] = -1*(w.x)*(a.x) - (w.y)*(a.y) - (w.z)*(a.z);
                            ...
                            float m_Far)
{
    ...
    float d = atanf(m_Angle);
    
    elements[0] = d/m_AspectRatio;     elements[4] = 0;   elements[8] = 0;   elements[12] = 0;
    elements[1] = 0;   elements[5] = d;   elements[9] = 0;   elements[13] = 0;
    elements[2] = 0;   elements[6] = 0;   elements[10] = (-1)*(m_Far + m_Near)/(m_Far - m_Near);     elements[14] = (-2 * m_Far * m_Near)/(m_Far - m_Near);
    elements[3] = 0;  elements[7] = 0;  elements[11] = -1; elements[15] = 1;
                      ...
                      float m_Far)
{
    ...
    elements[0] = (2/(m_Right - m_Left));    
    elements[12] = -1*(m_Right + m_Left)/(m_Right - m_Left);
    elements[5] = (2/(m_Top - m_Bottom));     
    elements[13] = -1*(m_Top + m_Bottom)/(m_Top - m_Bottom);
    elements[10] = (-2)/(m_Far - m_Near);  
    elements[14] = -1*(m_Far + m_Near)/(m_Far - m_Near);
}
